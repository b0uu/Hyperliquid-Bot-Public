...
def place_market_order(account, api_key, coin, buyOrSell, size, delay, revBuyOrSell, price):
...
#while current_volume_usd < TARGET_VOLUME:
    ...
    def loopMain(account, api_key, coin, buyOrSell, size, delay, revBuyOrSell, priceRoundedUp):
        
        while current_volume < TARGET_VOLUME:
            global positionSizeOne 
            positionSizeOne = get_random_num(config["position_size1_range_min"], config["position_size1_range_max"])
            place_market_order(account, api_key, coin, buyOrSell, size, delay, revBuyOrSell, priceRoundedUp)
    ...
    def loopTwo(account, api_key, coin, buyOrSell, size, delay, revBuyOrSell, priceRoundedDown):
        global current_volume
        global hold_time
        while current_volume < TARGET_VOLUME:
            global positionSizeTwo 
            positionSizeTwo = get_random_num(config["position_size1_range_min"], config["position_size1_range_max"])
            place_market_order(account, api_key, coin, buyOrSell, size, delay, revBuyOrSell, priceRoundedDown)
            #current_volume = config["position_size1"] + current_volume
            #current_volume_usd = current_volume * eth_mid_price
    ...
    thread1 = threading.Thread(target=loopMain, args=(account_1, config["account_1_api_key"], config["coin"], True, positionSizeOne, hold_time, False, math.ceil(float(specific_mid_prices.get('ETH')))))
    thread2 = threading.Thread(target=loopTwo, args=(account_2, config["account_2_api_key"], config["coin"], False, positionSizeTwo, hold_time, True, math.floor(float(specific_mid_prices.get('ETH')))))
